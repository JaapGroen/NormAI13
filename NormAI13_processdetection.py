...
def process_detection(image,detection):
    ...
    if HC_object['position'] == 'north':
        if LP_object['position'] == 'west':
            results['transformations'].append('No flips/rotations required')
        ...
        if LP_object['position'] == 'north':
            results['transformations'].append('Rotating the image 90 degrees')
        ...
        if LP_object['position'] == 'north':
            results['transformations'].append('Flipping the image vertically')
            image,detection = VerticalFlip(image, objects)
            results['transformations'].append('Rotating the image 270 degrees')
    ...
    if mean_LP > mean_noise:
        results['transformations'].append('Inverting the image')
        image = np.amax(image)-image
    else:
        results['transformations'].append('Inverting not needed')
    ...
    for object in objects:
        ...
        ax.add_patch(rect)
    plt.title('Position of all objects')
    results['image_objects'] = fig
